...
import { firestoreConnect, firestoreReducer, firebaseConnect, getFirebase } from 'react-redux-firebase'
...
class DishDetails extends React.Component{
    ...
    render (){
        ...
        const dish = this.props.dishes ? this.props.dishes.filter((dish)=>{return id === dish.id}) : null;
        ...
        let rates = dish
                    ...
                    {return(a + b)})/(dish[0].rating.length): 0;
        ...
        return (
            <div>
                ...
                <option  value = "Select Value" style = {{display: "none"}} disabled>Select Level</option></select> 
                ...
                <div>
                {
                       ingredients.map((ingredient,index)=>{
                           return(
                               <label  key = {index} > 
                                  <input className = "select-checkbox" type = "checkbox" value = {ingredient} className = "ingredients-drop-down" onChange = {this.changeIngredient} />
                                  {ingredient}
                               </label>
                      ...
                      <label  >
                       <input type = "checkbox" className = "select-checkbox" value ={ingredients} onChange = {this.selectAll}></input>
                       Select All                    
                      </label>
                </div>
              ...
              <button className = "count-button" onClick = {this.plusCount}>+</button>
                 <div>Rating:</div>    
                <div className="rating-container" style={{left: '100px'}}onMouseMove= {(e)=> {this.countRating(e)}} 
                 onClick={(e) => {this.onClickToStars(e)}} 
                 onMouseLeave={() => {this.mouseLeaving()}}>
                    ...
                    <img className ="dish-image" src = {dish && dish[0].image}></img>
